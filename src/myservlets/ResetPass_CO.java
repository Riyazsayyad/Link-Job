 ...
 */
@WebServlet("/ResetPass_CO")
public class ResetPass_CO extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try 
		{
			...
			if (lst.size()>0) 
			{
				...
				if(i>0)
				{
					response.sendRedirect("Intermediate.jsp?status=pass1");
				}
				
			}
			else 
			{
				response.sendRedirect("Intermediate.jsp?status=fail1");
